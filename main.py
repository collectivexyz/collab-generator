...
def process_video(video_path, window_size, output_path, padding_seconds=3):
    ...
    out = cv2.VideoWriter(f'{output_path}/output_{video_path.split("/")[-1]}', fourcc, fps, (width, height))
...
def main(video_paths, total_duration, output_path):
    for video_path in video_paths:
        fps = cv2.VideoCapture(video_path).get(cv2.CAP_PROP_FPS)
        window_size = min(int(total_duration / len(video_paths) * fps), int(fps*2))  # window size in frames
        start_frame, end_frame = process_video(video_path, window_size, output_path)
...
if __name__ == "__main__":
    ...
    output_path = './output'
    main(video_paths, total_duration, output_path)
